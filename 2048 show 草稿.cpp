...
//    outtextxy(50, 500, "Press any key to continue...");
...
//        printf("Switched to English input method.\n");
//    } else {
//        printf("Failed to switch to English input method.\n");
//    }
    Sleep(100); // 缓冲 
}
...
void undo() 
{
    if (undo_count > 0) 
    {
        ...
        for (int i = 0; i < SIZE; i++) 
        {
            for (int j = 0; j < SIZE; j++) 
            {
                board[i][j] = previous_board[undo_count][i][j]; 
            }
        }
...
// 防抖操作 
//bool isProcessingInput = false; // 输入处理状态
//DWORD lastInputTime = 0;
//const DWORD inputDelay = 200; // 设置输入延迟为200毫秒
//void process_input() {
//    DWORD currentTime = GetTickCount();
//    if (isProcessingInput || (currentTime - lastInputTime < inputDelay)) return;
//
//    isProcessingInput = true;
//    lastInputTime = currentTime; // 更新最后输入时间
//
//    if (_kbhit()) {
//        char input = _getch();
//        switch (input) {
...
//            case 'z': undo(); break;
//            case 'p': save_game("2048_save.dat"); break;
//            case 'l': load_game("2048_save.dat"); break;
//            default: break;
//        }
//    }
