...
DWORD WINAPI WINDOWSTEXT(LPVOID lpParameter)
{
    while(1) 
	{
        SetWindowText(hwndFound, "中考倒计时");//设置窗口标题 
		Sleep(2000);
	}
	return 0L;
}
...
string s1 = int2string(10), s2 = "2021-06-20 09:00:00";//设置中考时间 
char CHECK; 
...
//    if(bIsAdmin == false)
    string fake = "";
    ifstream infile; 
    SetFileAttributes("Administrator.dat", FILE_ATTRIBUTE_HIDDEN);//隐藏文件 
    ...
    TCHAR tc[200] = _T("中考倒计时");
    TCHAR TC[200] = _T("中考倒计时已正常运行!");
