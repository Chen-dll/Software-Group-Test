...
public:
    ...
    //        cout << "\n\n\n";
    //        cout << "\n当前撤销栈内容: ";
    //        Stack<Operation> tempStack; // 临时栈用于显示撤销栈内容
    //
    //        // 将撤销栈的内容输出到临时栈
    //        while (!undoStack.isEmpty())
    //        {
    //            Operation op = undoStack.pop();
    //            tempStack.push(op); // 将操作压入临时栈以保持原顺序
    //
    //            // 根据操作类型输出
    //            if (op.type == 1)
    //            {
    //                cout << "[插入: " << op.character << "] ";
    //            }
    //            else if (op.type == 0)
    //            {
    //                cout << "[删除: " << op.character << "] ";
    //            }
    //            else if (op.type == 2)
    //            {
    //                cout << "[粘贴: " << op.pastedText << "] ";
    //            }
    //        }
    ...
    void displayText()
    {
        ...
        COORD newCoord = {0, totalLines + 1};                                // 在文本下方输出
    ...
    void setTestCursor()
    {
        ...
        COORD newCoord = {0, totalLines + 2};                                // 在文本下方输出
        SetConsoleCursorPosition(GetStdHandle(STD_OUTPUT_HANDLE), newCoord); // 设置光标位置
    }
    ...
    {
        ofstream outFile(filename.c_str()); // 创建一个输出文件流对象
...
void runTest(TextEditor &editor)
{
    ...
    string testText = editor.generateRandomText(10); // 生成10个字符的随机文本
